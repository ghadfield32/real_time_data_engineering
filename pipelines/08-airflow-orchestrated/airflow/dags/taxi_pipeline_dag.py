...
with DAG(
    ...
    submit_flink_bronze = BashOperator(
        ...
        bash_command=(
            ...
            "-f /opt/flink/sql/01-create-kafka-source.sql && "
            "docker exec -T p08-flink-jobmanager /opt/flink/bin/sql-client.sh embedded "
            "-f /opt/flink/sql/02-create-iceberg-catalog.sql && "
            "docker exec -T p08-flink-jobmanager /opt/flink/bin/sql-client.sh embedded "
            "-f /opt/flink/sql/03-bronze-raw-trips.sql"
        ),
    )
    ...
    submit_flink_silver = BashOperator(
        ...
        bash_command=(
            "docker exec -T p08-flink-jobmanager /opt/flink/bin/sql-client.sh embedded "
            "-f /opt/flink/sql/04-silver-cleaned-trips.sql"
        ),
    )
