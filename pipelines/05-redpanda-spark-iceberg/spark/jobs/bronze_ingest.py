"""
Bronze Ingest: Redpanda -> Iceberg (Structured Streaming)

Reads raw taxi trip JSON events from the Redpanda topic `taxi.raw_trips`,
...
The broker address is configurable via the BROKER_URL environment variable,
defaulting to "redpanda:9092".
...
BROKER_URL = os.environ.get("BROKER_URL", "redpanda:9092")
...
def create_spark_session() -> SparkSession:
    ...
    return (
        ...
        .appName("Pipeline05_BronzeIngest")
...
def run_bronze_ingest(spark: SparkSession) -> None:
    """Read from Redpanda, parse JSON, and write to Bronze Iceberg table."""

    taxi_schema = get_taxi_schema()

    # -------------------------------------------------------------------------
    # Read from Redpanda (Kafka-compatible protocol)
    # -------------------------------------------------------------------------
    kafka_df = (
        spark.readStream
        .format("kafka")
...
def main():
    print("=" * 60)
    print("  Pipeline 05 — Bronze Ingest (Redpanda -> Iceberg)")
    print("=" * 60)
