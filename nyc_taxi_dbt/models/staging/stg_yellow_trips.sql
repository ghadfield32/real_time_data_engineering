{#
    ...
    Evolution through the learning weeks:
    - Week 2: Used read_parquet() directly
    - Week 3: Added dbt_utils.generate_surrogate_key() for trip_id
    - Week 4: Refactored to use source() via dbt-duckdb external sources

    This is the final (Week 4) version using source().
#}
...
renamed as (
    select
        -- surrogate key (Week 3+: requires dbt_utils package)
        -- includes fare_amount + total_amount to disambiguate adjustment records
