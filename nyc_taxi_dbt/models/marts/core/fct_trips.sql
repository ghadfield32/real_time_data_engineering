/*
    ...
    Joins trip metrics with dimension tables to create a wide,
    analysis-ready fact table. Uses incremental materialization
    with delete+insert strategy for efficient updates.

    On full refresh: loads all trips.
    On incremental run: loads only trips newer than the latest
    pickup_datetime already in the table.
*/
